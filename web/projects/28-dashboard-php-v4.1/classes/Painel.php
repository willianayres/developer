<?php
	...
	class Painel
	{
		...
		public static function loadJS($files, $pages = '', $geral = false, $cdn = false, $cdn_add = ''){
			$url = explode('/',@$_GET['url'])[0];
			if(!$geral && !$cdn){
				for($i=0;$i<count($pages);$i++){
					if($pages[$i] == $url){
						foreach($files as $key => $value){
							echo '<script src="'.INCLUDE_PATH_PAINEL.'js/'.$value.'.js"></script>';
						}
						break;
					}
				}
				...
				for($i=0;$i<count($files);$i++){
					if($files[$i]=='jquery' || $files[$i]=='constants')
						...
						echo '<script src="'.INCLUDE_PATH_PAINEL.'js/'.$files[$i].'.js"></script>';
				}
			}else if($geral && $cdn){
				$script = '<script src="'.$files[0].'" ';
				if($cdn_add != ''){
		...
		public static function loadCSS($files, $pages, $geral = false, $cdn = false, $cdn_add = ''){
			$url = explode('/',@$_GET['url'])[0];
			if(!$geral && !$cdn){
				for($i=0;$i<count($pages);$i++){
					if($pages[$i] == $url){
						foreach($files as $key => $value){
							echo '<link rel="stylesheet" href="'.INCLUDE_PATH_PAINEL.'css/'.$value.'.css" />';
						}
						break;
					}
				}
				...
				for($i=0;$i<count($files);$i++){
					if($files[$i]=='fontawesome')
						...
						echo '<link rel="stylesheet" href="'.INCLUDE_PATH_PAINEL.'css/'.$files[$i].'.css" />';
				}
			}else if($geral && $cdn){
				$script = '<link rel="stylesheet" href="'.$files[0].'" ';
				if($cdn_add != ''){

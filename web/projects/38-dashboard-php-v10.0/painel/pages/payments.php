...
<div class="box">
	...
	<div class="wraper-table">
		<table>
			...
			<?php
				$pending_payments = \Painel::getData('tb_admin.payments','*',' WHERE status = ? ORDER BY due ASC LIMIT '.($cur_pendent_page - 1) * $per_page.','.$per_page,array(0),1,1);
	...
	<div class="pagination">
		<?php
			...
			$tot_pages = ceil(count(\Painel::getData('tb_admin.payments','*',' WHERE status = ?',array(0),1,1)) / $per_page);
	...
	<div class="wraper-table">
		<table>
			...
			<?php
				$pay_payments = \Painel::getData('tb_admin.payments','*',' WHERE status = ? ORDER BY due ASC LIMIT '.($cur_pay_page - 1) * $per_page.','.$per_page,array(1),1,1);
	...
	<div class="pagination">
		<?php
			// Total pages is definid by ceiling the number of elementes on the table divided
			// by the number that you want to have showed per page.
			$tot_pages = ceil(count(\Painel::getData('tb_admin.payments','*',' WHERE status = ?',array(1),1,1)) / $per_page);
			for($i=1; $i<=$tot_pages; $i++){
				if($i == $cur_pay_page)

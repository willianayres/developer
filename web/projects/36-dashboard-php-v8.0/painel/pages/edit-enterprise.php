<?php
	if(isset($_GET['id'])){
		$id = (int)$_GET['id'];
		$enterprise = \Painel::getData('tb_admin.enterprises','*',' WHERE id = ?',array($id),0);
...
<div class="box">
	...
	<div class="wraper-table box-wraper-table">
		...
		<table>
			...
			<?php
				$buildings = \Painel::getData('tb_admin.buildings','*',' WHERE enterprise_id = ?',array($id),1,1);
				if(count($buildings)){
			...
			if(isset($_POST['register'])){
				...
				if($check){
					...
					if(\Painel::insertData($building)){
						...
						$building = \Painel::getData('tb_admin.buildings','*','',[],1,1);
						$building_id = $building[count($building)-1]['id'];
						// Iterate to insert all the images into the database.
						for($i=0; $i<count($img_names); $i++){
							// Check if the insertion was ok.
							if(!\Painel::insertData(array('table'=>'tb_admin.buildings-images','building_id'=>$building_id,'img'=>$img_names[$i]),0)){
								$upload = false;
								break;
							}
						}
				...
				// Everthing is ok.
				if($check && $upload){
					\Painel::redirectPage(INCLUDE_PATH_PAINEL.'edit-enterprise?id='.$id.'&registered');
				}
			}

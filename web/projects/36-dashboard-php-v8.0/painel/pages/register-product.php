		...
		<?php
			if(isset($_POST['register'])){
				...
				if($check){
					...
					if(\Painel::insertData($product,0)){
						// Get the product id that was inserted.						
						$product = \Painel::getData('tb_admin.storage','*','',[],1,1);
						$product_id = $product[count($product)-1]['id'];
						// Iterate to insert all the images into the database.
						for($i=0; $i<count($img_names); $i++){
							// Check if the insertion was ok.
							if(!\Painel::insertData(array('table'=>'tb_admin.storage-images','product_id'=>$product_id,'img'=>$img_names[$i]),0)){
								$upload = false;
								break;
							}
						}

<?php
	...
	$check_category = Painel::getData('tb_site.categories', '*', "WHERE slug = ?", array($url_notice[1]), false);
	if($check_category->rowCount() == 0)
		Painel::redirectPage(INCLUDE_PATH.'noticias');
	$check_category = $check_category->fetch();
	$post 			= Painel::getData('tb_site.notices', '*', 'WHERE slug = ? AND category_id = ?', array($url_notice[2], $check_category['id']), false);
	if($post->rowCount() == 0)
		Painel::redirectPage(INCLUDE_PATH.'noticias');
	$post 			= $post->fetch();
?>

<?php
	...
	if(isset($_GET['id'])){
		...
		$client = \Painel::getData('tb_admin.clients','*',' WHERE id = ?',array($id),0);
		if($client->rowCount() == 0){
			\Painel::alert('error', 'O cliente que você quer editar não existe!');
			die();
		}
		$client = $client->fetch();
	}else{
	...
	// Get the current page of paid payments.
	$cur_pay_page = isset($_GET['pay_page']) ? (int)$_GET['pay_page'] : 1;
	// Get the current page of pendent payments.
...
<div class="box">
	...
	<?php
		// Check if payment_action is set.
		if(isset($_POST['payment_action'])){
			// Get all the payment info.
			$client_id = $id;
			$payment_name = $_POST['payment_name'];
			// Put the value in the correct form.
			$payment_value = ($_POST['payment_value'] == '') ? '0.00' : \Painel::moneySubmit($_POST['payment_value']);
			...
			if($payment_name == '' || $payment_value = '' || $parcels == '' || $interval == '' || $initial_due == ''){
				...
				if(strtotime($initial_due) < strtotime(date('Y-m-d'))){
					...
					// Create and save the parcels in the Database.
					for($i=0; $i<$parcels; $i++){
						$due = strtotime($initial_due) + (($i * $interval) * (60*60*24));	
						$sql = \Painel::insertData(array('table'=>'tb_admin.payments','client_id'=>$client_id,'name'=>$payment_name,'value'=>$payment_value,'due'=>date('Y-m-d',$due),'status'=>0),0);
						if(!$sql){
							$check = false;
							break;
						}
	...
	<form method="post">
		...
		<div class="group">
			...
			<input type="text" name="payment_parcels"/>
		</div><!--group-->
		<div class="group">
			<label>Intervalo:</label>
			<input type="text" name="payment_interval"/>
		</div><!--group-->
		<div class="group">
			<label>Vencimento:</label>
			<input type="text" name="payment_due"/>
		</div><!--group-->
		<div class="group">
	...
	<?php
		// Check if the paid button was trigged.
		if(isset($_GET['pay'])){
			// Update the Database.
		...
		// Check if the email button was trigged.
		if(isset($_GET['email'])){
			$parcel_id = (int)$_GET['parcel'];
			$client_id = (int)$_GET['email'];
			// Generate a cookie to prevent span.
	...
	<div class="wraper-table">
		<table>
			...
			<?php
				// Get the pendent payments from the Database.
				$pending_payments = \Painel::getData('tb_admin.payments','*',' WHERE status = ? AND client_id = ? ORDER BY due ASC LIMIT '.($cur_pendent_page - 1) * $per_page.','.$per_page,array(0,$id),1,1);
			...
			?>
				<tr <?php echo $style;?>>
					...
					<td><?php echo 'R$ '.\Painel::money($value['value']);?></td>
	...
	<div class="pagination">
		<?php
			...
			$tot_pages = ceil(count(\Painel::getData('tb_admin.payments','*',' WHERE status = ? AND client_id = ?',array(0,$id),1,1)) / $per_page);
	...
	<div class="wraper-table">
		<table>
			...
			<?php
				// Get the paid payments from the Database.
				$pay_payments = \Painel::getData('tb_admin.payments','*',' WHERE status = ? AND client_id = ? ORDER BY due ASC LIMIT '.($cur_pay_page - 1) * $per_page.','.$per_page,array(1,$id),1,1);
	...
	<div class="pagination">
		<?php
			// Total pages is definid by ceiling the number of elementes on the table divided
			// by the number that you want to have showed per page.
			$tot_pages = ceil(count(\Painel::getData('tb_admin.payments','*',' WHERE status = ? AND client_id = ?',array(1,$id),1,1)) / $per_page);
			for($i=1; $i<=$tot_pages; $i++){
				if($i == $cur_pay_page)

...
<div class="box">
	...
	<?php
		if(isset($_GET['delete'])){
			...
			$id = (int)$_GET['delete'];
			$imgs = Painel::getData('tb_admin.storage-images','*',' WHERE product_id = ?',array($id),true,true);
		...
		$amount = Painel::getData('tb_admin.storage','*',' WHERE amount = ?',array(0),false);
	...
	<div class="boxes">
		<?php
			$products = Painel::getData('tb_admin.storage','*',$query,'',true,true);
			$products_search = count(Painel::getData('tb_admin.storage','*',$query_search,'',true,true));
			if(isset($_GET['search'])){
				echo '<div class="search-result"><p>Foram encontrados <strong>'.$products_search.'</strong> resultado(s).</p></div>';
			}
			foreach($products as $value){
				$product_img = Painel::getData('tb_admin.storage-images','*',' WHERE product_id = ? LIMIT 1',array($value['id']),true);
		?>
	...
	<div class="pagination">
		<?php
			...
			$tot_pages = ceil(count(Painel::getData('tb_admin.storage','*',$query_search,'',true,true)) / $per_page);
...
<?php }else{ ?>
	...
	<?php
		...
		$products = Painel::getData('tb_admin.storage','*',$query,array(0),true,true);
		if(count($products) > 0)
	...
	<div class="boxes">
		<?php
			foreach($products as $value){
				$product_img = Painel::getData('tb_admin.storage-images','*',' WHERE product_id = ? LIMIT 1',array($value['id']),true)['img'];
		?>
			<div class="boxes-wraper">

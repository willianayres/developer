...
// Check if is a set a valid id.
	if(isset($_GET['id'])){
		...
		$product = \Painel::getData('tb_admin.storage','*',' WHERE id = ?',array($id),0);
...
<div class="box">
	...
	<?php
		...
		if(isset($_GET['deleteimage'])){
			...
			$img = \Painel::getData('tb_admin.storage-images','*',' WHERE id = ?',array($id_img),1);		
		...
		$product_imgs = \Painel::getData('tb_admin.storage-images','*',' WHERE product_id = ?',array($id),1,1);
	?>
		...
		<?php
			if(isset($_POST['action'])){
				...
				if($check){
					...
					if(\Painel::updateData($product,0)){						
						// Iterate to insert all the images into the database.
						if($_FILES['imgs']['name'][0] != ''){
							for($i=0; $i<count($img_names); $i++){
								// Check if the insertion was ok.
								if(!\Painel::insertData(array('table'=>'tb_admin.storage-images','product_id'=>$id,'img'=>$img_names[$i]),0)){
									$upload = false;
									break;
								}
							}

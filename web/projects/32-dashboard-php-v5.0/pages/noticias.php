<?php
	...
	if(!isset($url[2])){
		$category = Painel::getData('tb_site.categories','*',"WHERE slug = ?",array(@$url[1]),true);
?>
		...
		<div class="notices left">
			<div class="header">
				<?php
					...
					$tot_pages = Painel::getData('tb_site.notices', '*', $qry, $arra, false);
					...
					$notices = Painel::getData('tb_site.notices', '*', $query, $arr, true, true);
				?>
			</div><!--header-->
			<?php foreach ($notices as $key => $value) { 
				//$category_name = Painel::selectData('tb_site.categories', 'slug', "id = ?", array($value['category_id']), true);
				$category_name = Painel::getData('tb_site.categories','slug','WHERE id = ?', array($value['category_id']),false);
			...
			<div class="box">
				...
				<a href="<?php echo INCLUDE_PATH; ?>noticias/<?php echo $category_name; ?>/<?php echo $value['slug']; ?>">Ler mais</a>
			</div><!--box-->
			<?php }?>

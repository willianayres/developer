...
// Check if is a set a valid id.
	if(isset($_GET['id'])){
		...
		$building = Painel::getData('tb_admin.buildings','*',' WHERE id = ?',array($id),false);
...
<div class="box">
	...
	<?php
		...
		if(isset($_GET['deleteimage'])){
			...
			$img = Painel::getData('tb_admin.buildings-images','*',' WHERE id = ?',array($id_img),true);		
			...
			$building_imgs = Painel::getData('tb_admin.buildings-images','*',' WHERE building_id = ?',array($id),true,true);
	...
	<div class="pagination">
		<?php
			...
			$tot_pages = ceil(count(Painel::getData('tb_admin.buildings-images','*',' WHERE building_id = ?',array($id),true,true)) / $per_page);
					...
					if(Painel::updateData($building)){
						...
						if($_FILES['imgs']['name'][0] != ''){
							for($i=0; $i<count($img_names); $i++){
								// Check if the insertion was ok.
								if(!Painel::insertData(array('table'=>'tb_admin.buildings-images','building_id'=>$id,'img'=>$img_names[$i]),false)){
									$upload = false;
									break;
								}
							}

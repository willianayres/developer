...
<div class="box">
	...
	<?php
		if(isset($_GET['delete'])){
			...
			$id = (int)$_GET['delete'];
			$enterprise = Painel::getData('tb_admin.enterprises','*',' WHERE id = ?',array($id),true);

			$buildings = Painel::getData('tb_admin.buildings','id',' WHERE enterprise_id = ?',array($id),true,true);
			foreach($buildings as $key => $value){
				$building_imgs = Painel::getData('tb_admin.buildings-images','*',' WHERE building_id = ?',array($value['id']),true,true);
	...
	<div class="boxes" id="sortable">
		<?php
			$enterprise = Painel::getData('tb_admin.enterprises','*',$query,'',true,true);
			$enterprise_search = count(Painel::getData('tb_admin.enterprises','*',$query_search,'',true,true));
				...
				<div class="boxes-wraper-inside">
					...
					<div class="box-single-divide">
						<div class="body">
							...
							<div class="buttons">
								<a class="see" href="<?php pathPainel(); ?>edit-enterprise/<?php echo $value['id'];?>"><i class="far fa-eye"></i> Editar</a>
	...
	<div class="pagination">
		<?php
			...
			$tot_pages = ceil(count(Painel::getData('tb_admin.enterprises','*', $query_search,'',true,true)) / $per_page);

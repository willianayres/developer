document.addEventListener('DOMContentLoaded',()=>{
    ...
    // Get the player contnet. //
    const player = document.querySelector('#player');
    // Start with the player-one. //
    let  curPlayer = 1;
    for(var i=0, len=sqr.length; i<len; i++)
    (function(index){
    	// Add an onclick to each square in your grid. //
        if(i < 42){
	        sqr[i].onclick = function(){
	            // If the square below your current square is taken, you can go ontop of it. //
	            if(sqr[index + 7].classList.contains('taken') && !sqr[index].classList.contains('taken')){
	                if(curPlayer === 1){
	                    sqr[index].classList.add('taken');
	                    sqr[index].classList.add('player-one');
	                    // Change the player. //
	                    curPlayer = 2;
	                    player.innerHTML = curPlayer;
	                }else if (curPlayer === 2){
	                    sqr[index].classList.add('taken');
	                    sqr[index].classList.add('player-two');
	                    // Change the player. //
	                    curPlayer = 1;
	                    player.innerHTML = curPlayer;
	                }
	                // If the sqaure below your current swqaure is not taken, you can't go there. //
	            }else alert('Não pode vir aqui!!!');
	        }
    	}
    })(i);
    ...
    function gameOver(){
        ...
        for(let j=0; j<winMoves.length; j++){
            const sq1 = sqr[winMoves[j][0]];
            const sq2 = sqr[winMoves[j][1]];
            const sq3 = sqr[winMoves[j][2]];
            const sq4 = sqr[winMoves[j][3]];
            //now check those arrays to see if they all have the class of player-one
            if(sq1.classList.contains('player-one') && sq2.classList.contains('player-one') &&
                sq3.classList.contains('player-one') && sq4.classList.contains('player-one')){
                    // If they do, player-one is passed as the winner. //
                    result.innerHTML = 'Player Um Ganhou !!!';
                    // Remove ability to change result. //
            }else if(sq1.classList.contains('player-two') && sq2.classList.contains('player-two') &&
                       sq3.classList.contains('player-two') && sq4.classList.contains('player-two')){
                    ...
                    result.innerHTML = 'Player Dois Ganhou !!!';
            }
        }
    }
    // Add an event listener to each square that will trigger the gameOver function on click. //
    sqr.forEach(sq => sq.addEventListener('click', gameOver));
});